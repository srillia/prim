...
本文内容比较长，如果直接想clone项目体验直接进入[项目体验](#12-项目体验) [prim项目下载](#4prim-项目) ,文本从介绍webSocket是什么开始，然后开始介绍这个项目，以及在Nginx中配置域名做webSocket的转发，然后介绍如何搭建一个分布式系统。
...
#### 1.2 项目体验
- [项目地址 prim](https://github.com/link1st/prim)
...
发送图片消息，发送消息者的客户端需要先把图片上传到文件服务器，上传成功以后获得图片访问的 URL，然后由发送消息者的客户端需要将图片 URL 发送到 prim，prim 图片的消息格式发送给目标客户端，消息接收者客户端接收到图片的 URL 就可以显示图片消息。
...
语言消息、和视频消息和图片消息类似，都是先把文件上传服务器，然后通过 prim 传递文件的 URL，需要注意的是部分消息涉及到隐私的文件，文件访问的时候需要做好鉴权信息，不能让非接收用户也能查看到别人的消息内容。
...
```
git clone git@github.com:link1st/prim.git
# 或
git clone https://github.com/link1st/prim.git
```
- 修改项目配置
...
### 7.1 说明
- 参考本项目源码
- [prim v1.0.0 单机版Im系统](https://github.com/link1st/prim/tree/v1.0.0)
- [prim v2.0.0 分布式Im系统](https://github.com/link1st/prim/tree/v2.0.0)
...
- 重启以后请求，验证是否符合预期:
 ...
 查看请求是否落在两个项目上
 实验两个用户分别连接不同的项目(prim和prim1)是否也可以相互发送消息
...
github 搜:link1st 查看项目 prim

[https://github.com/link1st/prim](https://github.com/link1st/prim)


### 赞助商

感谢[JetBrains](https://www.jetbrains.com/?from=prim)对本项目的支持！
<p align="center">
    <a href="https://www.jetbrains.com/?from=prim">
